...
from rotated.assigners.task_aligned import RotatedTaskAlignedAssigner, TaskAlignedAssigner


def test_rotated_task_aligned_assigner_forward():
    """Test RotatedTaskAlignedAssigner forward pass."""
    ...
    pred_boxes = torch.rand(batch_size, num_anchors, 5, device=device) * 100  # [cx, cy, w, h, angle]
    ...
    gt_boxes = torch.rand(batch_size, num_gts, 5, device=device) * 100  # [cx, cy, w, h, angle]
    ...
    assert assigned_boxes.shape == (batch_size, num_anchors, 5)
